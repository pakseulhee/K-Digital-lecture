...
select deptno "#", dname "부서명", loc "위치" from dept;

--2. 교수 테이블(professor)을 사용하여 '홍길동(교수)' 와 같이 교수명(직급) 형식으로 하고 컬럼명은 교수님 으로 출력하세요.
select name ||'('|| position ||')' "교수님" from professor;
...
select name, pay, nvl(bonus,0) "bonus", pay*12+nvl(bonus,0) "연봉" from professor where deptno = 101;
...
select name, substr(jumin, 3, 2) month, decode(substr(jumin, 3, 2),01,'1/4분기',02,'1/4분기',03,'1/4분기',
                                                                    04,'2/4분기',05,'2/4분기',06,'2/4분기',
                                                                    07,'3/4분기',08,'3/4분기',09,'3/4분기','4/4분기')"분기"from student;
...
--14. Professor 테이블에서 심슨 교수와 같은 입사일에 입사한 교수 중에서 
...
--15. emp2 테이블을 사용하여 전체 직원 중 과장 직급의 최소 연봉자보다 연봉이 높은 사람의 이름과 직급, 연봉을 출력하세요. 
--단, 연봉 출력 형식은 천 단위 구분 기호와 원 표시를 하세요.
select name, emp_type, to_char(pay, '9,999,999,999')||'원'
from emp2
